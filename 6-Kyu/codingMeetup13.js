...
function isLanguageDiverse(list) {
  ...
  list.forEach(dev => {
    if(diversity.has(dev.language)){
      let num = diversity.get(dev.language)
      diversity.set(dev.language, num + 1)
    } else {
      diversity.set(dev.language, 1)
    }
  })
    let maxNum = Math.max(...diversity.values())
    let minNum = Math.min(...diversity.values())
    return maxNum <= minNum * 2 ? true : false
}
